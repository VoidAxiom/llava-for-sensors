...
**The bug.** VOI-210's first 10 of 15 ablation runs on real CWRU all produced bit-identical `val_f1 = test_f1 = 0.500`. Diagnostic: `data/cwru.py::build_split(seed=0)` had been shipped (in VOI-205, PR #32) with file-grouped stratification — windows from the same `.mat` file kept together. With only 4 files per class and an 80/10/10 split on 16 total files, sklearn's `train_test_split` cannot satisfy the stratification constraint AND class coverage simultaneously; the val/test 50/50 of 4 holdout files (one per class after the 80/20 stratified pass) falls back to non-stratified random (per the `can_stratify_hold` guard — N=1 per class can't be stratified), and `seed=0` produces val={ORF, BF} only, test={Normal, IRF} only. Both eval splits cover only half the label space, so macro-F1 is capped at (1+1+0+0)/4 = 0.5 regardless of model quality.
