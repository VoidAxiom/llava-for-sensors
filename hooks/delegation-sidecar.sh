...
• Delegation-first, by disposition. You're the director: spawn an `implementer` subagent per packet in its own worktree (`scripts/worktree-new.sh` + Task tool, `subagent_type: implementer`). The implementer dispatches codex exec for code writes, runs gates, drives local `/code-review --effort high` (Claude Opus 4.7 — the cross-family local reviewer; `/codex:review` is deprecated), commits, pushes, opens the PR, drives the `@codex review` eye-emoji loop. Multiple impls in parallel is the right move when work naturally parallelizes (disjoint surfaces); serial is fine when it doesn't. Reserve your own hands for spec authoring, the pre-PR scope check + audit-trail check, and the merge-time re-gate. If genuine parallelism is honestly blocked (e.g. author-gated), that's completely okay — just say so plainly.

• Coordination — Linear plans, GitHub delivers. Linear is the planning ledger: one tracked issue per work unit, in its milestone, with a description detailed enough to build straight from; set it In Progress before launch; the PR says `Closes VOI-N`. GitHub is the delivery ledger: squash-merge only on a head-pinned Codex verdict on the current head + zero unresolved threads + CLEAN mergeStateStatus + green CI. Trigger reviews with one of the two accepted forms — (a) bare standalone `@codex review` for an initial review, or (b) `@codex review` on the first line followed by a "Changes since last review" / "Not changed deliberately" rationale block per implementer.md §8e for a re-review. Any other `@codex` mention spawns a phantom cloud task. Treat the Codex connector as a reader — act on its findings text and verify repo state, never its self-reported commits.
